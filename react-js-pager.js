...
const Pager = /*#__PURE__*/(0, _react.forwardRef)((props, ref) => {
  ...
  const resizeObserver = (0, _react.useRef)(); // resize event listiner for pager wrapper element.

  const isAnimationRunning = (0, _react.useRef)(false); // used to stop resize event when the animation is still running to prevent unstabel behaviour.
  // props
  ...
  const animationStyle = (_props$animationStyle = props.animationStyle) !== null && _props$animationStyle !== void 0 ? _props$animationStyle : 'scroll'; // 'opacity', 'scale', 'scaleX', 'scaleY', 'rotateX', 'rotateY', blur
  ...
  const checkTypes = () => {
    ...
    if (orientation !== 'horizontal' && orientation !== 'vertical') console.error('react-js-pager: props.orientation has invalid value.');
    ...
    if (typeof wheelScrollAnimation !== 'boolean') console.error('react-js-pager: props.wheelScrollWithAnimation has invalid value.');
    if (!new Set(['scroll', 'opacity', 'scale', 'scaleX', 'scaleY', 'rotateX', 'rotateY', 'blur']).has(animationStyle)) console.error('react-js-pager: props.animationStyle has invalid value.');
    ...
    const allProps = new Set(['initialPage', 'orientation', 'touchGestures', 'wheelScroll', 'wheelScrollWithAnimation', 'animationStyle', 'perspective', 'duration', 'loop', 'showScrollbar', 'ease', 'onNavigationStart', 'onPageSelected', 'onAnimation', 'onPagerScroll', 'wrapperStyle', 'id', 'className', 'children']);
  ...
  (page, withAnimation, type) => {
    ...
    if (navigateTo < maxScroll) {
      ...
      if (withAnimation) (0, _requestAnimationNumber.requestNum)({
        to: 1,
        duration,
        easingFunction
      }, s => {
  ...
  (0, _react.useLayoutEffect)(() => {
    if (isFirstRender) {
      orientation === 'vertical' ? adjustHeight() : adjustWidth();
      resizeObserver.current = new ResizeObserver(onResizeHandle).observe(pagerRef.current);
      changePage(initialPage, false);
  ...
  (0, _react.useEffect)(() => {
    ...
    return () => {
      ...
      pagerObserver === null || pagerObserver === void 0 ? void 0 : pagerObserver.unobserve(pagerEl);
    }; // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
